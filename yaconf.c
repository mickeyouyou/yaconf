...
static void php_yaconf_simple_parser_cb(zval *key, zval *value, zval *index, int callback_type, void *arg) /* {{{ */ {
	...
	if (callback_type == ZEND_INI_PARSER_ENTRY) {
		...
		if (!(Z_STRLEN_P(key) > 1 && Z_STRVAL_P(key)[0] == '0')
			...
			if (zend_hash_index_find(Z_ARRVAL_P(arr), idx, (void **)pzval) == FAILURE) {
				php_yaconf_hash_init(&rv, 8);
				zend_hash_index_update(Z_ARRVAL_P(arr), idx, &rv, sizeof(rv), (void **)pzval);
			} 
		} else {
			char *seg, *ptr;
...
static void php_yaconf_ini_parser_cb(zval *key, zval *value, zval *index, int callback_type, void *arg) /* {{{ */ {
	...
	if (callback_type == ZEND_INI_PARSER_SECTION) {
		...
		zend_symtable_update(Z_ARRVAL_P(arr), skey, strlen(skey), &active_ini_file_section, sizeof(active_ini_file_section), NULL);
					...
					for (i = 0; i < ndir; i++) {
						...
						if (zend_symtable_find(ini_containers,
							...
							zend_symtable_update(ini_containers,
									namelist[i]->d_name, p - namelist[i]->d_name, &result, sizeof(result), NULL);
						}
